...
plotLMER.fnc(dijkstra.lmer, pred="OS" ,ylim =c(-2.07, -1.8), xlabel = "OS", ylabel = "-1000/RT", main = "");rug(dijkstra$OS);points(x[length(x)],y[length(y)], col="red")
...
plotLMER.fnc(dijkstra.lmer.poly2, pred="poly(OS, 2, raw = T)" ,ylim =c(-2.07, -1.8), xlabel = "OS", ylabel = "-1000/RT", main = ""); rug(dijkstra$OS)
...
dijkstra.gam = bam(invRT  ~     
                     s(OS) +
                     s(Frequency) +
                     s(Trial, Participant, bs="fs", m=1) + # factor smooth for trial (continuous variable) and participant (factor variable)
...
# compare m1 and m2
compareML(dijkstra.gam, dijkstra.gam2) # With the by-participant random slopes for Frequency and OS, the model improved (fREML score decreases from 1063 to 1056). We also test whether the factor Ident will improve the model.
